...
SELECT s.shopname, c2.name
FROM shops s
JOIN (SELECT name, shops_id FROM cats WHERE name = "Murzik") c2
ON s.id = c2.shops_id;
...
LEFT JOIN cats c
ON s.id = c.shops_id
WHERE s.id NOT IN(SELECT shops_id FROM cats WHERE name = "Murzik" OR name = "Zuza");
